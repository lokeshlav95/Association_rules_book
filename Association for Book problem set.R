...
library(arulesViz)
attach(book)
str(book) #to check the type of data set 
class(book)
# making rules using apriori algorithm 
#  changing support and confidence values to obtain different rules
?apriori
# Apriori algorithm for support =0.04,confidence =0.8
Arules=apriori(as.matrix(book),parameter=list(support=0.04,confidence=0.8))
Arules # set of 94 rules


inspect((sort(Arules,by="lift"))) # Sort the Highest lift ratio in decending order 

#the highest lift ratio is 4.14 and we concluded that the if the person buys the  {,ItalArt}  and then he/she  would like to buy {Artbks} also we suggest the vender/client this rule is  first and top most rule
# in same lift ratio 4.14 another combination of books,if the person buys {cooksbks,Italcook} then also  would like to buy {ArtBks}
